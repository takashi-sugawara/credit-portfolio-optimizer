...
def run_sensitivity_analysis(df, target_value, pd_mult, opt_mode):
    ...
    parameters = {
        t('Customer Payment Potential C', '顧客決済ポテンシャル C'): ('PotentialC', 0.1),
        t('Credit Utilization Responsiveness k', '与信利用反応度 k'): ('AlphaK', 0.1),
        t('Probability of Default PD', 'デフォルト確率 PD'): ('PD', 0.1)
    }
...
scenario_map = {
    t('Neutral Scenario (Default Prob x1.0)', '中立シナリオ (デフォルト確率 1.0倍)'): ('Neutral', 1.0, "🔵"),
    t('Optimistic Scenario (Default Prob x0.7)', '楽観シナリオ (デフォルト確率 0.7倍)'): ('Optimistic', 0.7, "🟢"),
    t('Pessimistic Scenario (Default Prob x1.5)', '悲観シナリオ (デフォルト確率 1.5倍)'): ('Pessimistic', 1.5, "🔴")
}
